...
export function ContainerLogsDialog({ containerId, containerName, runtimeSessionId }: { containerId: string; containerName: string; runtimeSessionId: string }) {
  const logs = useContainerLogs({ containerId, runtimeSessionId });
  const [open, setOpen] = useState(false);
  useEffect(() => { if (open) void logs.refetch(); }, [open]);
  return <Dialog trigger={<Button>View logs for {containerName}</Button>} title={`Logs: ${containerName}`} description="Newest retained container output. Logs do not update automatically." open={open} onOpenChange={setOpen}>
