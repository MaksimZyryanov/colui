...
export function ContainerLogsDialog({ containerId, containerName, runtimeSessionId, compact = false }: { containerId: string; containerName: string; runtimeSessionId: string; compact?: boolean }) {
  ...
  return <Dialog trigger={compact ? <Button iconOnly aria-label={`View logs for ${containerName}`} title="View logs">≡</Button> : <Button>View logs for {containerName}</Button>} title={`Logs: ${containerName}`} description="Newest retained container output. Logs do not update automatically." open={open} onOpenChange={setOpen}>
