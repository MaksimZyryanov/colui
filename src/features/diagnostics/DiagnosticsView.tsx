...
export function DiagnosticsView() {
  ...
  return <main className="page"><header className="page-heading"><div><p className="eyebrow">System health</p><h1>Diagnostics</h1><p>Runtime, inventory, registry, and recovery status.</p></div></header>
    {diagnostics.isLoading ? <div role="status" aria-label="Loading diagnostics">Loading diagnostics...</div> : diagnostics.isError || !data ? <Alert variant="destructive">Diagnostics unavailable. Registry-backed Projects remain available offline.</Alert> : <>
      ...
      <section aria-labelledby="definitions-heading"><h2 id="definitions-heading">Definitions</h2><Card><p>Generation {data.definitions.generation}</p>{data.definitions.profiles.length ? <ul>{data.definitions.profiles.map(profile => <li key={profile.profileId} aria-label={`Definition profile ${profile.profileId}`}><strong>{profile.profileId}</strong>: {profile.definition ? title(profile.definition.state) : 'Unchecked'}{profile.definition?.issues.length ? <ul aria-label={`Issues for profile ${profile.profileId}`}>{profile.definition.issues.map((issue, index) => <li key={`${issue.field ?? 'general'}-${index}`}>{issue.field ? <><code>{issue.field}</code>: </> : null}{issue.message}</li>)}</ul> : <p>No definition issues.</p>}{errorDetails(profile.error)}</li>)}</ul> : <p>No retained definitions.</p>}</Card></section>
      <section aria-labelledby="journal-heading"><h2 id="journal-heading">Journal</h2><Card>{data.journal.entries.length ? <ol>{data.journal.entries.map(entry => <li key={entry.sequence} aria-label={`Journal entry ${entry.sequence}`}><time dateTime={entry.timestamp}>{entry.timestamp}</time> · {title(entry.kind)} · {subjectLabel(entry.subject)}{entry.errorCode ? <> · <code>{entry.errorCode}</code></> : null}<dl className="diagnostic-grid"><dt>Severity</dt><dd>{title(entry.severity)}</dd><dt>Runtime session</dt><dd>{entry.runtimeSessionId ? <code>{entry.runtimeSessionId}</code> : 'No runtime session'}</dd></dl><p>{entry.message}</p></li>)}</ol> : <p>No journal entries.</p>}</Card></section>
    </>}
  </main>;
}
