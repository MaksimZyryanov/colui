...
describe('DiscoverySection', () => {
  ...
  it('retains deterministic feedback for concurrent same-type candidate requests', async () => {
    ...
    expect(await screen.findByRole('alert', { name: `Registration candidate-b (${b.candidateId}) failed` })).toHaveTextContent('candidate_stale');
    expect(await screen.findByRole('status', { name: `Registration candidate-a (${a.candidateId}) succeeded` })).toBeVisible();
    expect(screen.getByRole('alert', { name: `Registration candidate-b (${b.candidateId}) failed` })).toHaveTextContent('Candidate B changed');
  });

  it('distinguishes concurrent outcomes for candidates with duplicate project names', async () => {
    let finishA!: (value: unknown) => void;
    let rejectB!: (error: unknown) => void;
    const a = { ...candidate('1', 'new_unambiguous'), composeProjectName: 'shared-app' };
    const b = { ...candidate('2', 'new_unambiguous'), composeProjectName: 'shared-app' };
    mockBackend.setResponseOverride('list_discovery_candidates', { candidates: [a, b], autoRegistrationEnabled: false });
    mockBackend.setResponseOverride('register_candidate', new Promise(resolve => { finishA = resolve; }));
    const user = userEvent.setup();
    render(<QueryClientProvider client={client}><DiscoverySection runtimeSessionId={session} inventoryGeneration={4} registryHealth={null} /></QueryClientProvider>);
    await user.click((await screen.findAllByRole('button', { name: 'Register shared-app' }))[0]);
    await waitFor(() => expect(mockBackend.getInvocations().filter(call => call.command === 'register_candidate')).toHaveLength(1));
    mockBackend.setResponseOverride('register_candidate', new Promise((_resolve, reject) => { rejectB = reject; }));
    await user.click(screen.getAllByRole('button', { name: 'Register shared-app' })[1]);
    rejectB(new AppErrorException({ code: 'candidate_stale', operation: 'register_candidate', subject: { kind: 'candidate', id: b.candidateId }, message: 'Second candidate changed', retryable: false }));
    finishA({ id: '00000000-0000-0000-0000-000000000001', revision: 1, displayName: 'shared-app', composeProjectName: 'shared-app', workingDirectory: '/work/app', registrationOrigin: 'discovered' });
    expect(await screen.findByRole('status', { name: `Registration shared-app (${a.candidateId}) succeeded` })).toBeVisible();
    expect(await screen.findByRole('alert', { name: `Registration shared-app (${b.candidateId}) failed` })).toHaveTextContent('Second candidate changed');
  });
  ...
  it.each([
    ...
    const expectedAction = command === 'register_candidate' || command === 'ignore_candidate' ? `${action} new_unambiguous-app (${'1'.repeat(64)})` : action;
  ...
  it('announces discovery mutation successes', async () => {
    ...
    expect(await screen.findByRole('status', { name: `Registration new_unambiguous-app (${'1'.repeat(64)}) succeeded` })).toBeVisible();
    await user.click(screen.getByRole('button', { name: /ignore new_unambiguous-app/i }));
    expect(await screen.findByRole('status', { name: `Ignore new_unambiguous-app (${'1'.repeat(64)}) succeeded` })).toBeVisible();
    await user.click(screen.getByRole('switch', { name: /automatic registration/i }));
