...
export function DiscoverySection({ runtimeSessionId, inventoryGeneration, registryHealth }: { runtimeSessionId: string | null; inventoryGeneration: number; registryHealth: RegistryHealth | null }) {
  ...
  const register = (request: RegisterCandidateRequest) => { if (registering.has(request.candidateId) || ignoring.has(request.candidateId)) return; setRegistering(current => new Set(current).add(request.candidateId)); void mutations.register.mutateAsync(request).catch(() => undefined).finally(() => setRegistering(current => { const next = new Set(current); next.delete(request.candidateId); return next; })); };
  const ignore = (candidateId: string) => { if (registering.has(candidateId) || ignoring.has(candidateId)) return; setIgnoring(current => new Set(current).add(candidateId)); void mutations.ignore.mutateAsync({ candidateId }).catch(() => undefined).finally(() => setIgnoring(current => { const next = new Set(current); next.delete(candidateId); return next; })); };
